...
double tau, m, C, v_eq, L, dt, d, t, o, w, pi;
...
output1=fopen("Trafic3_w_dif_posicions", "w");
...
for(i=t_c; i<N; i++){
	...
	v[i][0]=v_eq*(1-0.8*sin(dt*w*0.05*(i-t_c)));
	//printf("%lf\n", x[i][0]);
}
...
for(i=t_c; i<t_c+T; i++){
	fprintf(output1,"%lf \t %lf \t %lf \t %lf\n", x[i][0]-x[i][1],x[i][1]-x[i][2],x[i][2]-x[i][3],x[i][3]-x[i][4]);
}
fprintf(output1, "\n");
fprintf(output1, "\n");
}
